...
<?php
if(isset($_GET['id'])){
$qry = $conn->query("SELECT * FROM 1sem_schedules where id= ".$_GET['id']);
			...
			<div class="row">
				<div class="col-md-6">
					<div class="form-group">
						<label for="" class="control-label">Sample</label>
					...
					<div class="form-group">
						...
						?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($Csub_id) && $Csub_id == $row['id'] ? 'selected' : '' ?>><?php echo ($row['course']) ?></option>
						<?php endwhile; ?>
						</select> 
					</div>
					...
					<div class="form-group">
						...
						<select name="title" id="" class="custom-select select2">
							<option value="0">All</option>
						<?php 
							$subject_global = $conn->query("SELECT *, concat(CPE1_SUBJ_Code,'- ',CPE1_SUBJ_Descript) as subj_name FROM subject_global ");
							while($row= $subject_global->fetch_array()):
						?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($title) && $title == $row['id'] ? 'selected' : '' ?>><?php echo ($row['subj_name']) ?></option>
						<?php endwhile; ?>
						</select> 
					</div>
